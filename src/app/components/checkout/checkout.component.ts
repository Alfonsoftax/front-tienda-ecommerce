...
export class CheckoutComponent implements OnInit {
  ...
  obtener_una_direccion() {
    this._guestService.obtener_direccion_principal_cliente(localStorage.getItem('_id'),this.token).subscribe(
      response=>{
        if(response.data = undefined) {
          this.direccionPrincipal = undefined;        

        }else {
          this.direccionPrincipal = response.data;        
        }
        this.load_data = false;
      }
    );
  ...
  calcular_carrito(){
    ...
    this.total_pagar_const = this.subtotal;
        ...
        //Codigo de tienda antigua. Borrar cuando lo actualice
        this.total_pagar = parseInt(this.subtotal.toString()) + parseInt(this.envio.toString());
  }

  select_direccion_envio(item:any){
    this.envio_gratis = false;
    this.direccion_envio = item;
    this.direccion_envio.principal = true;
    if(this.direccion_envio.pais != 'España'){
    ...
    }else if(this.direccion_envio.pais == 'España'){
